...
#[tokio::test]
async fn text_round_trip_with_tungstenite_engine() {
...
#[tokio::test]
async fn binary_round_trip_with_tungstenite_engine() {
    let server = spawn_websocket_echo().await;
    let client = Client::builder().build().expect("client");
    let request = ws_request(&format!("ws://{}/", server.addr()));
...
#[tokio::test]
async fn server_initiated_close_with_tungstenite_engine() {
